...
using Server.Spells;
using Server.Spells.Zulu.ClericSpells;
namespace Server.Gumps.Zulugumps
{

    public class holybookgump : Gump
    {

        Mobile test;

        public holybookgump(Mobile owner, bool[] array)
            : base(100, 0)
        {
            try {
                test = owner;
            ...
            if (array[16])
            {
                this.AddLabel(xName2, yName2 += 20, 66, @"Holy Gate");
