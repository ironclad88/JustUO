...
namespace Server.Gumps.RaceGumps
{
    ...
    public class HumanGump : Gump
    {
        ...
        public HumanGump(Mobile owner)
            : this(owner, owner, ResurrectMessage.Generic, false)
        {
        }


        public HumanGump(Mobile owner, Mobile healer, ResurrectMessage msg, bool fromSacrifice)
            : this(owner, healer, msg, fromSacrifice, 0.0)
        {
        }

        public HumanGump(Mobile owner, Mobile healer, ResurrectMessage msg, bool fromSacrifice, double hitsScalar)
            : base(100, 0)
        {
            ...
            this.AddHtml(50, 55, 300, 140, "Enter the world with the Human race?", true, true);
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            from.CloseGump(typeof(HumanGump));
            from.Frozen = false;
            if (info.ButtonID == 1 || info.ButtonID == 2)
            {
                from.MoveToWorld(new Point3D(1475, 1645, 20), Map.Trammel); // move player to Britain center
               // from.Hue = 33877; // not using any hue for human, only using the standard hue
