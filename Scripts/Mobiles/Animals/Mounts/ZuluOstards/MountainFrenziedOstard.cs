...
{
    [CorpseName("an mountain frenzied ostard corpse")]
    public class MountainFrenziedOstard : BaseMount
    {
        [Constructable]
        public MountainFrenziedOstard()
            : this("a mountain frenzied ostard")
        {
        }

        [Constructable]
        public MountainFrenziedOstard(string name)
            : base(name, 0xDA, 0x3EA5, AIType.AI_Animal, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            this.Hue = 1160;
            this.stableName = "a mountain frenzied ostard";
