...
{
    [CorpseName("an emerald frenzied ostard corpse")]
    public class EmeraldFrenziedOstard : BaseMount
    {
        [Constructable]
        public EmeraldFrenziedOstard()
            : this("a emerald frenzied ostard")
        {
        }

        [Constructable]
        public EmeraldFrenziedOstard(string name)
            : base(name, 0xDA, 0x3EA5, AIType.AI_Animal, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            this.Hue = 1159;
            this.stableName = "a emerald frenzied ostardd";
