...
namespace Arya.Auction
{
    ...
    public class AuctionLog
    {
        ...
        {
            if (AuctionSystem.Running && AuctionSystem.EnableLogging)
            {
        ...
        public static void WriteNewAuction(AuctionItem auction)
        {
            ...
            try
            {
                ...
                m_Writer.WriteLine("# Created on {0} at {1}", DateTime.UtcNow.ToShortDateString(),
                    DateTime.UtcNow.ToShortTimeString());
                m_Writer.WriteLine(
                    "# Owner : {0} [{1}] Account: {2}", auction.Owner.Name, auction.Owner.Serial,
                    auction.Account.Username);
                m_Writer.WriteLine(
                    "# Expires on {0} at {1}", auction.EndTime.ToShortDateString(), auction.EndTime.ToShortTimeString());
                m_Writer.WriteLine(
                    "# Starting Bid: {0}. Reserve: {1}. Buy Now: {2}",
                    auction.MinBid,
                    auction.Reserve,
                    auction.AllowBuyNow ? auction.BuyNow.ToString(CultureInfo.InvariantCulture) : "Disabled");
                m_Writer.WriteLine("# Owner Description : {0}", auction.Description);
                m_Writer.WriteLine("# Web Link : {0}", auction.WebLink ?? "N/A");
                ...
                if (auction.Creature)
                {
                    ...
                    m_Writer.WriteLine(
                        "# Type : {0}. Serial : {1}. Name : {2} Hue : {3}",
                        auction.Pet.GetType().Name,
                        auction.Pet.Serial,
                        auction.Pet.Name ?? "Unnamed",
                        auction.Pet.Hue);
                    m_Writer.WriteLine("# Statuette Serial : {0}", auction.Item.Serial);
                ...
                else
                {
                    ...
                    for (int i = 0; i < auction.ItemCount; i++)
                    {
                        ...
                        m_Writer.WriteLine(
                            "# {0}. {1} [{2}] Type {3} Hue {4}", i, info.Name, info.Item.Serial,
        ...
        public static void WriteBid(AuctionItem auction)
        {
            ...
            {
                m_Writer.WriteLine(
                    "> [{0}] Bid Amount : {1}, Mobile : {2} [{3}] Account : {4}",
                    auction.ID,
                    auction.HighestBidValue,
                    auction.HighestBidder.Name,
                    auction.HighestBidder.Serial,
                    auction.HighestBidder.Account.Username);
            }
            catch
            {
            }
        ...
        public static void WritePending(AuctionItem auction, string reason)
        {
            ...
            {
                m_Writer.WriteLine(
                    "] [{0}] Becoming Pending on {1} at {2}. Reason : {3}",
                    auction.ID,
        ...
        public static void WriteEnd(AuctionItem auction, AuctionResult reason, Mobile m, string comments)
        {
            ...
            try
            {
                m_Writer.WriteLine("## Ending Auction {0}", auction.ID);
                m_Writer.WriteLine("# Status : {0}", reason);

                if (m != null)
                {
                    m_Writer.WriteLine("# Ended by {0} [{1}], {2}, Account : {3}", m.Name, m.Serial, m.AccessLevel,
                        m.Account.Username);
                }
        ...
        public static void WriteViewItem(AuctionItem auction, Mobile m)
        {
            ...
            {
                m_Writer.WriteLine(
                    "# Vieweing item [{0}] Mobile: {1} [{2}], {3}, Account : {4}",
                    auction.ID,
                    m.Name,
                    m.Serial,
                    m.AccessLevel,
        ...
        public static void WriteReturnItem(AuctionItem auction, Mobile m)
        {
            ...
            {
                m_Writer.WriteLine(
                    "# Returning item [{0}] Mobile: {1} [{2}], {3}, Account : {4}",
                    auction.ID,
                    m.Name,
                    m.Serial,
                    m.AccessLevel,
