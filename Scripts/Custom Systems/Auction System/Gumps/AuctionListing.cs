...
namespace Arya.Auction
{
    ...
    public class AuctionListing : Gump
    {
        ...
        public AuctionListing(Mobile m, IEnumerable<AuctionItem> items, bool searchEnabled, bool returnToAuction,
            int page)
            : base(50, 50)
        {
        ...
        public AuctionListing(Mobile m, IEnumerable<AuctionItem> items, bool searchEnabled, bool returnToAuction)
            : this(m, items, searchEnabled, returnToAuction, 0)
        {
        }
        ...
        private void MakeGump()
        {
            ...
            if (m_EnableSearch)
            {
                AddLabel(305, 120, LabelHue, AuctionSystem.ST[16]);
                AddButton(270, 120, 4005, 4006, 1, GumpButtonType.Reply, 0);
                m_Buttons.Add(1);
            }

            // Sort: BUTTON 2
            AddLabel(395, 120, LabelHue, AuctionSystem.ST[17]);
            ...
            if (m_List.Count > 0)
            {
                ...
                AddLabel(360, 95, RedHue, string.Format(AuctionSystem.ST[18], m_Page + 1, (m_List.Count - 1)/10 + 1));

                AddLabel(70, 120, RedHue, string.Format(AuctionSystem.ST[19], m_List.Count));
            }
            else
            {
                AddLabel(70, 120, RedHue, AuctionSystem.ST[20]);
            }
            ...
            if (m_List.Count > 0)
            {
                ...
                {
                    AuctionItem item = m_List[m_Page*10 + i];

                    AddButton(115, 153 + i*20, 5601, 5605, 10 + i, GumpButtonType.Reply, 0);
                    m_Buttons.Add(10 + i);
                    AddLabelCropped(140, 150 + i*20, 260, 20, LabelHue, item.ItemName);
                }
            }
            ...
            {
                AddLabel(355, 360, LabelHue, AuctionSystem.ST[22]);
            ...
            if (m_Page > 0)
            {
                AddLabel(180, 360, LabelHue, AuctionSystem.ST[21]);
                AddButton(280, 360, 4014, 4015, 4, GumpButtonType.Reply, 0);
                m_Buttons.Add(4);
            }

            // Close: BUTTON 0
            AddLabel(115, 360, LabelHue, AuctionSystem.ST[7]);
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                default:
                {
                    ...
                    AuctionItem item = m_List[index];

                    if (item != null)
                    {
                        if ((!item.Expired || item.Pending) &&
                            (AuctionSystem.Auctions.Contains(item) || AuctionSystem.Pending.Contains(item)))
                        {
