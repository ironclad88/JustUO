...
namespace Arya.Auction
{
    ...
    {
        private const int OutbidHue = 2107;
        private const int SoldHue = 2125;

        private int m_GoldAmount;
        private bool m_Used; // Additional anti-duping check
        ...
        public AuctionGoldCheck(AuctionItem auction, AuctionResult result)
        {
            ...
            m_GoldAmount = result != AuctionResult.BuyNow ? auction.HighestBid.Amount : auction.BuyNow;
            ...
            switch (result)
            {
                ...
                default:
                    throw new Exception(string.Format("{0} is not a valid reason for an auction gold check", result));
            }
        }

        public override string ItemName
        {
            get { return string.Format("{0} Gold Coins", m_GoldAmount); }
        }

        public override Item DeliveredItem
        {
            get { return new Gold(m_GoldAmount); }
        }

        #region Serialization

        public AuctionGoldCheck(Serial serial)
            : base(serial)
        {
        }
        ...
        public override bool Deliver(Mobile to)
        {
            ...
            if (!Banker.Deposit(m_Owner, m_GoldAmount))
            {
