...
namespace Arya.Auction
{
    ...
    public class Bid
    {
        ...
        public void Outbid(AuctionItem auction)
        {
            ...
            if (!m_Mobile.Backpack.TryDropItem(m_Mobile, check, false))
            {
        ...
        {
            string owner = m_Mobile != null && m_Mobile.Account is Account
                ? String.Format(
                    "{0} [Account: {1} - Serial: {2}]", m_Mobile.Name, ((Account) m_Mobile.Account).Username,
                    m_Mobile.Serial)
                : "None";

            content.AppendLine("- {0}\t{1}", m_Amount, owner);
        }
    }
}